...
def GMM(features, num_reps, area):
    ...
    plt.plot(components, bics_mean, 'k', color='deepskyblue')
    plt.fill_between(components, bics_mean-bics_stds, bics_mean+bics_stds,
                    alpha=0.5, edgecolor='cadetblue', facecolor='powderblue', label = 'SEM')
    plt.title(area + " Mean BIC Value per Component Size")
    plt.legend()
    plt.xlabel('Number of Components for GMM')
    plt.ylabel('Mean BIC Value')
    ...
    plt.plot(components, aics_mean, 'k', color='deepskyblue')
    plt.fill_between(components, aics_mean-aics_stds, aics_mean+aics_stds,
                    alpha=0.5, edgecolor='cadetblue', facecolor='powderblue', label = 'SEM')
    plt.title(area + " Mean AIC Value per Component Size")
    plt.legend()
    plt.xlabel('Number of Components for GMM')
